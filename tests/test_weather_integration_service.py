...
def test_get_city_info_returns_city_when_open_meteo_finds_city(
    monkeypatch,
) -> None:
    ...
    assert result.name == "Indaiatuba"
    assert result.country_code == "BR"
    assert result.latitude == -23.08842
    assert result.longitude == -47.2119
